...
export async function retrieveJourneys(usernames: string[]): Promise<RetrievedJourney[]> {
    ...
    try {
      ...
      `,
        ...
        return result.records.map((record) => ({
            ...
            reputationScore: (record.get("reputationScore") as number) ?? 0,
            experienceIds: record.get("experienceIds") as string[],
        }));
    } finally {
